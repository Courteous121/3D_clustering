...
/**
 * @brief 通过识别框进行坐标变换得到相应的点云
 *
 * @param csv_loc csv点云文件
 * @param cloud_loc 得到目标点云的pcd文件
 * @return 世界坐标点
 */
...
void locate::convertStr2pcd(vector<vector<double>> str, string cloud_loc)
{
    ofstream outfile(cloud_loc);
    outfile << "# .PCD v0.7 - Point Cloud Data file format" << endl;
    outfile << "VERSION 0.7" << endl;
    ...
    outfile << "TYPE F F F" << endl;
    outfile << "COUNT 1 1 1" << endl;
    outfile << "WIDTH " << str.size() << endl;
    outfile << "HEIGHT 1" << endl;
    outfile << "VIEWPOINT 0 0 0 1 0 0 0" << endl;
    outfile << "POINTS " << str.size() << endl;
    outfile << "DATA ascii" << endl;
    for (int i = 0; i < str.size(); i++)
    {

        outfile << str[i][0] << " " << str[i][1] << " " << str[i][2] << endl;
        // cout<<result[i][0]<<" "<<result[i][1]<<" "<<result[i][2]<<endl;
    }
}
