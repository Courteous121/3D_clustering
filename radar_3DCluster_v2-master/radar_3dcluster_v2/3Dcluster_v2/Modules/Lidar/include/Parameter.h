...
struct BTParam
{
    int BEGIN_END;  // 前中期分界线
    int MIDDLE_END; // 中后期分界线
    int MINE_WARN;  // 落矿提前提醒时间
    int BUFF_WARN;  // Buff开启提前提醒时间
    int LOW_HP;     // 低血量阈值

    BTParam(const string &param);
};
...
struct IconParam
{
    ...
    int base_middle_HP;
    int base_low_HP;
    int watcher_middle_HP;
    int watcher_low_HP;
    int outpost_middle_HP;
    int outpost_low_HP;

    IconParam(const string &param);
};

// 关键点相关参数
struct KeyPointParam
{
    vector<Point2f> calibration_point; // 标定地图用点
    Point outpost_point;               // 前哨站点
    Point buff_point;                  // 神符点

    KeyPointParam(const string &param);
};

// 外挂辅助相关参数
struct ArtifactParam
{
    float distance_rect_size; // 激光雷达测距框框的边长大小(单位为像素)

    ArtifactParam(const string &param);
};

// 激光雷达点云图相关参数
struct CalParam
{
    int undistcoeff_flag; // 是否使用畸变矫正标志位
    int polynomial_flag;  // 多项式算法标志位
    // 多项式参数
    double ax;
    double bx;
    double cx;
    ...
    CalParam(const string &param);
};

// 标定矩形相关参数
struct RectParam
{
    Rect mine_cut = Rect(0, 0, -1, -1);
    Rect outpost_cut = Rect(0, 0, -1, -1);
    Rect buff_cut = Rect(0, 0, -1, -1);
};
...
extern IconParam icon_param;
extern KeyPointParam key_point_param;
extern ArtifactParam artifact_param;
extern CalParam cal_param;
extern RectParam rect_param;
...
typedef struct
{
    ...
    int buff_count = NULL;                                  // 能量机关计时器
    int buff_start_time = NULL;                             // buff开始时间
    ...
    bool camera_config_flag = false;                        // 相机配置菜单开启标志位
...
struct MapShadow
{
    int shadow_time = -1; //残影效果倒计时
    Point last_location;  //上一次的位置
};

//获得区域的轮廓信息
void getArea(vector<vector<Point>> &design_area);
//将区域规划写入黑板
void WriteArea(void);
//清空位置信息
