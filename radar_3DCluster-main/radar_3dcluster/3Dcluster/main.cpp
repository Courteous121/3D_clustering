...
pcl::PointCloud<pcl::PointXYZ>::Ptr deletePlanet(pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr)
{
    ...
    int i = 0, nr_points = (int)in_cloud_ptr->points.size();
    while (in_cloud_ptr->points.size() > 0.5 * nr_points)
    {
        ...
        //  更新显示点云
        //  viewer.updatePointCloud(in_cloud_ptr, "cloud1");
...
vector<vector<Point2d>> locate2pic(PointCloud<PointXYZ>::Ptr cluster, vector<PointIndices> ecel)
{
    ...
    //     {
    //         cout << "(" << zhxin[i][j].x << "," << zhxin[i][j].y << "," << zhxin[i][j].z << ")" << endl;
    //     }
    // }

    // cout << "zhxin size is:" << zhxin.size() << endl;
    imgpoint.resize(ecel.size());
    for (int i = 0; i < ecel.size(); i++)
    {
...
int main(int argc, char **argv)
{
    ...
    string cloud_location = "/home/alyssa/save_point/test_point/test26.pcd";
    loc.convertcsv2pcd(csv_location, cloud_location);
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ>(cloud_location, *cloud) == -1) //*打开点云文件
    {
        PCL_ERROR("Couldn't read file test_pcd.pcd\n");
        return (-1);
    }
    cout << "cloud size is:" << cloud->points.size() << endl;
    double t7 = time7.toc();

    // show pcd files out
    // show3Dpoint(cloud);
    time1.tic();
    cloud2 = delete3DNear(cloud);
    cloud3 = delete3Dheight(cloud2);
    double t1 = time1.toc();

    time3.tic();
    cloud4 = delete3Dleftright(cloud3);
    double t3 = time3.toc();

    time4.tic();
    cloud5 = deletePlanet(cloud4);
    double t4 = time4.toc();
    ...
    vcds.setInputCloud(cloud5);        // 设置输入点云
    ...
    cloud6 = ig_cloud.EucliCluster(cloud_downSample, objp);
    getDis getd;
    vector<Eigen::Vector4f> zxgroup = getd.getZhiXin(objp);
    ...
    // showClussterInCloud(cloud3, cloud4);
